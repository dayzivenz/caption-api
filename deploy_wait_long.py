...
# Delete current service, try fresh deploy
...
# Deploy fresh via githubRepoDeploy
print("=== githubRepoDeploy ===")
dep = """mutation {
...
r3 = requests.post(api, json={"query": svc_q2}, headers=headers, timeout=10)
for svc in r3.json().get("data",{}).get("project",{}).get("services",{}).get("edges",[]):
    ...
    r4 = requests.post(api, json={"query": dom}, headers=headers, timeout=10)
    print(f"Domain: https://{r4.json().get('data',{}).get('serviceDomainCreate',{}).get('domain','N/A')}")

# Watch long
print("\n=== Watch (60s intervals x 10) ===")
for i in range(10):
    time.sleep(60)
    r5 = requests.post(api, json={"query": svc_q2}, headers=headers, timeout=10)
    for svc in r5.json().get("data",{}).get("project",{}).get("services",{}).get("edges",[]):
        ...
        r6 = requests.post(api, json={"query": dq}, headers=headers, timeout=10)
        for dep2 in r6.json().get("data",{}).get("deployments",{}).get("edges",[]):
            n = dep2["node"]
            print(f"  [{i+1}/10] {n['id'][:12]}... {n['status']} url={n.get('url','N/A')}")
            ...
            if n["status"] == "SUCCESS":
                ...
                r7 = requests.post(api, json={"query": bq}, headers=headers, timeout=10)
                for log in r7.json().get("data",{}).get("buildLogs",[]):
                    msg = log.get("message","").strip()
                    if msg and "scheduling" not in msg.lower():
